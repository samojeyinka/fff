...
import {   chatStart,
  ...
  updateChat,
  updateChatSuccess,
  updateChatFailure} from "../../../../redux/chat/chatSlice.js"
  import formatTime from 'format-hours';
  import { toast } from "react-toastify";
import { getDownloadURL, getStorage, ref, uploadBytesResumable } from "firebase/storage";
import { app } from "../../../../firebase";
import { Empty } from "antd";
import { formatDateTime } from "date-formater-df";
...
function Index() {
  ...
  const chats = [
...

    {
      ...
      firstName: "Olawumi",
      lastName: "Fillese",
      lastMessage: "I'm done with it sir",
      time: "3w",
      seen: true,
    },
  ...
  useEffect(() => {
    const getChats = async () => {
      try {
        const res = await axios.get(`http://localhost:8080/api/conversations/${currentUser._id}`);
...
useEffect(()=>{
  const getChat = async() => {
    try {
      const res = await axios.get(`http://localhost:8080/api/conversation/${currentChat}`)
      setReceiverId((res.data.participants[1]))
      setMessages(res.data.messages)
      setChatImages(res.data.messages || " ")

      // res.data.messages.forEach((ms)=>(
      //     console.log(ms.photo)
      // ))
    } catch (error) {
      console.log(error)
    }
...
useEffect(()=>{
  const getReceiver = async() => {
    try {
      const res = await axios.get(`http://localhost:8080/api/user/user-details/${recieverId}`)
      setReceiver(res.data)
      console.log("recievr", res.data)
    } catch (error) {
      console.log(error)
    }
  }
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setPhoto(downloadURL);
          console.log("photo link", downloadURL);
...
const handleEmojiModal = () => {
 ...
 emojiModal.style.display = "inherit"
  } else{
            ...
            <div className="chat__lists w-full border bg-[#FFFFFF] p-5 md:h-[100vh] md:w-[20rem]">
              ...
              <div className="chat__lists__boxes mt-4 flex flex-col gap-3">
                {conversations.map((chat) => (
                  ...
                  >
                    <div>
                      <img src={chat.participants[1] && chat.participants[1].profilePicture} width="48px" />
                    </div>
                    <div>
                      <h1 className="font-semibold text-[#171C1B]">
                        {/* {chat.firstName + " " + chat.lastName}{" "} */}
                        {chat.participants[1] && chat.participants[1].firstName + " " + chat.participants[1].lastName}
                      </h1>
                      <p className="text-[.9rem] text-[#465352]">
                        {chat.messages[chat.messages.length - 1] && chat.messages[chat.messages.length - 1].text || "No message yet"}
                      </p>
                    </div>
              ...
              <div className="chat__room">
                <div className="chat__room__nav flex justify-between border-b-2 px-8">
                  <div className="flex h-[5rem] cursor-pointer  items-center gap-2">
                    <div>
                      <img src={reciever && reciever.profilePicture} width="48px" />
                    </div>
                ...
                <div className="chat__room__chats h-full bg-[#f5f5f5]">
                  ...
                  {messages.map((message) => (
                         <div className={message.sender._id === currentUser._id ? "send ml-[50%] flex items-center gap-3" : "receive flex items-center gap-3"} key={message._id}>
                         <img src={message.sender.profilePicture} width="40px" />
                         <div className="flex flex-col gap-3">
                           <div
                             className={message.sender._id === currentUser && currentUser._id ? "w-[18rem] bg-[#7295B8] p-5 text-white" : "w-[18rem] bg-white p-5"}
                             style={{
                           ...
                           <img
                    src={message.photo || null }
                    style={{ width: "100%", height: "100%" }}
                  />
                           <small className="text-[#758A89]">{formatTime(new Date(message.createdAt))}</small>
                         </div>
                       </div>
                  ))
                        }
                ...
                {/* The emoji box starts here */}
                {/* <div 
                id="emojiModal"
                className="w-[auto] h-[auto]  absolute bottom-10 right-0" 
                style={{"zIndex":6,"display":"none"}}>
                <EmojiPicker />
                </div> */}
<div  className="w-[auto] h-[auto]  absolute bottom-10 right-0" >
{showPicker && (
          <Picker pickerStyle={{ width: "100%" }} onEmojiClick={onEmojiClick} />
                ...
                <div className="chat__room__tools flex h-[14rem] w-full items-center bg-[#f5f5f5]">
                  ...
                  <form className="chat__room__tools__box m-auto flex h-[11rem] w-[95%] flex-col gap-2 bg-white" onSubmit={handleSubmit}>
                    <textarea
                  ...
                  <div className="recent_files flex flex-col gap-2 border-b-2 px-3 py-2">
                    <div className="recent_files_nav flex items-center justify-between">
                      <div className="left flex items-center gap-1">
                        <h4 className="text-[1.03rem] font-bold">
                          Recent Files
                        </h4>
